...
export function PlansPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setSuccess("Nova versão publicada. Contratos existentes continuam preservados na versão anterior.");
      setShowForm(false);
      setForm(emptyForm());
      await load();
    } catch (caught) {
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {showForm ? (
        <form onSubmit={submit} className="space-y-6 rounded-2xl border border-[#DDD8CF] bg-white p-5 shadow-sm sm:p-7">
          <div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-black">{plans.some((plan) => plan.code === form.code) ? "Nova versão do plano" : "Novo plano"}</h2><p className="mt-1 text-sm text-[#626866]">Defina a oferta que ficará disponível para novas propostas.</p></div><button type="button" onClick={() => setShowForm(false)} aria-label="Fechar" className="rounded-lg p-2 hover:bg-gray-100"><X className="h-5 w-5" /></button></div>
      ...
      {loading ? <MetricGridSkeleton count={3} /> : plans.length === 0 ? <section className="rounded-2xl border border-dashed bg-white p-12 text-center"><Layers3 className="mx-auto h-10 w-10 text-[#B66E45]" /><h2 className="mt-3 font-black">Crie o primeiro plano comercial</h2><p className="mt-1 text-sm text-[#626866]">Escolha os produtos, o período de teste e os valores que serão oferecidos.</p>{hasPlatformPermission("platform.billing.manage") ? <button onClick={startNew} className="mt-4 rounded-xl bg-[#202322] px-4 py-2.5 text-sm font-bold text-white">Criar plano</button> : null}</section> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{plans.map((plan) => { const price = (plan.billing_plan_prices || []).find((item: any) => item.active) || plan.billing_plan_prices?.[0]; return <article key={plan.id} className={`rounded-2xl border bg-white p-5 ${plan.active ? "border-[#B66E45]/50 shadow-sm" : "border-[#DDD8CF] opacity-70"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-widest text-[#B66E45]">Versão {plan.version}</p><h2 className="mt-1 text-xl font-black">{plan.name}</h2></div><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${plan.active ? "bg-emerald-100 text-emerald-800" : "bg-gray-100 text-gray-600"}`}>{plan.active ? "Disponível" : "Versão anterior"}</span></div><p className="mt-2 min-h-10 text-sm text-[#626866]">{plan.description || "Sem descrição comercial."}</p><div className="mt-5 text-2xl font-black">{price ? money.format(price.amount_cents / 100) : "Preço a definir"}</div><p className="mt-1 text-xs text-[#626866]">{cycleLabel(price?.cycle)} · {billingLabel(price?.billing_type)} · teste de {plan.trial_days || 0} dias</p><div className="mt-4 border-t pt-4 text-sm text-[#626866]">{plan.billing_plan_solutions?.map((item: any) => item.solutions?.name).filter(Boolean).join(" · ") || "Nenhum produto associado"}</div>{hasPlatformPermission("platform.billing.manage") ? <button onClick={() => startVersion(plan)} className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[#B66E45]"><PencilLine className="h-4 w-4" /> Criar nova versão</button> : null}{!plan.active ? <div className="mt-3 flex items-center gap-1 text-xs text-[#777D7A]"><Archive className="h-3.5 w-3.5" /> Contratos existentes continuam nesta versão.</div> : null}</article>; })}</div>}
    </div>
  );
}
