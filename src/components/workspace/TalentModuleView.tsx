...
import { Briefcase, Server, Key, ArrowLeft, Terminal } from "lucide-react";
...
export function TalentModuleView({ tenant, user, onBack }: Props) {
  
  const tenantCtx = useTenant();
  const roleKeys = tenantCtx.roles.map((r: any) => r.key);
  
  
  const isAdmin = tenantCtx.hasPermission('talent.job.create') || roleKeys.includes('TENANT_ADMIN');
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
        ...
        <span>/</span>
        <span className="font-medium text-[#202322]">Ordum Talentos</span>
      </div>

      <div className="rounded-3xl border border-[#D98C32]/30 bg-white p-6 sm:p-8 shadow-xs relative overflow-hidden">
        <div className="absolute top-0 right-0 w-48 h-48 bg-[#FFF1DD] rounded-full blur-3xl pointer-events-none" />
        <div className="relative flex flex-col sm:flex-row gap-6">
          <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-2xl bg-[#FFF1DD] text-[#D98C32] font-extrabold shadow-xs">
            <Briefcase className="w-8 h-8" />
          </div>
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-2xl sm:text-3xl font-black text-[#202322]">Ordum Talentos</h1>
              <span className="px-2.5 py-1 bg-[#D98C32]/10 text-[#D98C32] text-[10px] font-bold uppercase rounded-full">Solução Ativa</span>
            </div>
            <p className="text-sm text-[#626866] max-w-2xl leading-relaxed">
              Gestão inteligente de recrutamento e seleção (ATS). Criação de vagas, portal de carreiras, 
              triagem de currículos e funil de processos seletivos.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-dashed border-[#DDD8CF] bg-[#FAF8F3]/50 p-8 sm:p-12 text-center space-y-6">
        ...
        <div className="max-w-xl mx-auto space-y-3">
          ...
          <p className="text-sm text-[#626866] leading-relaxed">
            ...
            está preparado para receber o sistema existente de Talentos por meio da arquitetura de integração já estabelecida.
          </p>
        </div>
        ...
        <div className="max-w-2xl mx-auto text-left bg-white p-6 rounded-xl border border-[#DDD8CF]/60 shadow-sm">
          <h3 className="text-sm font-bold text-[#202322] mb-4 flex items-center gap-2">
            <Server className="w-4 h-4 text-[#D98C32]" /> Ponto de Montagem do Sistema
          </h3>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="bg-gray-50 p-3 rounded-lg border border-gray-100 font-mono text-xs text-[#202322] overflow-x-auto">
                <span className="text-[#D98C32]">tenant</span>: {JSON.stringify({ id: tenant.id, slug: tenant.slug }, null, 2)}<br/>
                <span className="text-[#D98C32]">user</span>: {JSON.stringify({ id: user.id, role: user.role }, null, 2)}
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-[#626866] flex items-start gap-2">
                ...
                O sistema de Talentos utilizará o contexto do tenant injetado pelo Workspace. 
